        ...
        <div className="container py-12">
          <div className="mx-auto max-w-2xl rounded-2xl border bg-card p-8 shadow-lg">
            <div className="mb-6 flex flex-col items-center gap-3 text-center">
              ...
              <h2 className="font-display text-2xl font-bold">{t("req.selectCity")}</h2>
              <p className="text-muted-foreground">
                {t("req.selectCityHint")}
              </p>
            </div>
    ...
    <div className="min-h-screen bg-gradient-surface">
      ...
      <div className="container py-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="font-display text-2xl font-bold md:text-3xl">{t("req.title")}</h1>
            <p className="mt-1 text-muted-foreground">
              {sorted.length} {t("req.activeCount")}
              {priorityCity && (
              ...
              {!priorityCity && (
                <button onClick={handleChangeCity} className="ml-2 text-xs text-primary hover:underline">
                  <MapPin className="mr-0.5 inline h-3 w-3" />{t("req.selectCityShort")}
                </button>
              )}
            </p>
          </div>
          ...
          <div className="flex gap-2">
            {(effectiveRole === "user" || effectiveRole === "company") && user && (
              <Link to="/requests/new">
                <Button variant="hero">
                  <Plus className="mr-1.5 h-4 w-4" /> {t("req.post")}
                </Button>
              </Link>
            )}
            ...
            {effectiveRole === "worker" && (
              <Button variant="outline" className="pointer-events-none gap-1.5">
                <HardHat className="h-4 w-4" /> {t("req.workerHint")}
              </Button>
            )}

            {effectiveRole === "admin" && (
              <Button variant="outline" className="pointer-events-none gap-1.5">
                <ShieldCheck className="h-4 w-4" /> {t("req.adminHint")}
              </Button>
            )}
          ...
          <Popover open={filtersOpen} onOpenChange={setFiltersOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline" className="relative gap-2">
                ...
                <span className="hidden sm:inline">{t("req.filters")}</span>
                {activeFilterCount > 0 && (
            ...
            <PopoverContent className="w-80 space-y-4" align="end">
              <div className="flex items-center justify-between">
                <h3 className="font-display font-semibold">{t("req.filters")}</h3>
                {activeFilterCount > 0 && (
                  <Button variant="ghost" size="sm" className="h-auto gap-1 px-2 py-1 text-xs" onClick={clearFilters}>
                    <X className="h-3 w-3" /> {t("req.clearAll")}
                  </Button>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-muted-foreground">{t("req.category")}</Label>
                <Select value={selectedCategory || "__all__"} onValueChange={(v) => setSelectedCategory(v === "__all__" ? null : v)}>
                  <SelectTrigger>
                    <SelectValue placeholder={t("req.allCategories")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__all__">{t("req.allCategories")}</SelectItem>
                    {categories.map((cat) => (
                      <SelectItem key={cat.name} value={cat.name}>
                        {cat.icon} {tCategory(cat.name)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {/* City */}
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-muted-foreground">{t("req.city")}</Label>
                <Select value={selectedCity || "__all__"} onValueChange={(v) => setSelectedCity(v === "__all__" ? null : v)}>
                  <SelectTrigger>
                    <SelectValue placeholder={t("req.allCities")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__all__">{t("req.allCities")}</SelectItem>
                    {cities.map((city) => (
              ...
              {/* Min Rating */}
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-muted-foreground">{t("req.minRating")}</Label>
                <StarRating
                  rating={minRating}
                  size="md"
                  interactive
        ...
        {!loading && sorted.length === 0 && (
          <div className="py-20 text-center text-muted-foreground">
            <p className="text-lg font-medium">{t("req.noFound")}</p>
            <p className="mt-1 text-sm">{t("req.noFoundHint")}</p>
          </div>
        )}
      </div>
