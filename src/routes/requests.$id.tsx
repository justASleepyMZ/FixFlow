    ...
    return (
      <div className="min-h-screen bg-gradient-surface">
        ...
        <div className="container py-20 text-center">
          <h1 className="text-2xl font-bold">{t("rd.notFound")}</h1>
          <Link to="/requests"><Button variant="ghost" className="mt-4">{t("rd.back")}</Button></Link>
        </div>
        <Footer />
      </div>
    );
      ...
      <div className="container py-8">
        <Link to="/requests">
          <Button variant="ghost" size="sm" className="mb-4 gap-1">
            <ArrowLeft className="h-4 w-4" /> {t("rd.back")}
          </Button>
        </Link>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="accent">{tCategory(request.category)}</Badge>
                <Badge variant="status">{tStatus(request.status)}</Badge>
              </div>
              ...
              {request.address && (
                <p className="mt-2 text-sm text-muted-foreground"><MapPin className="inline h-3.5 w-3.5 mr-1" />{t("rd.address")} {request.address}</p>
              )}
              {(request.desired_start_date || request.desired_end_date) && (
                <div className="mt-3 flex items-center gap-2 rounded-lg bg-primary/5 px-3 py-2 text-sm">
                  ...
                  <span className="font-medium text-foreground">{t("rd.desiredPeriod")}</span>
              ...
              <Card className="border-secondary/50 shadow-card">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-lg">
                    <HardHat className="h-5 w-5" /> {t("rd.submitOffer")}
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <Label htmlFor="offer-price">{t("rd.proposedPrice")}</Label>
                  ...
                  <div>
                    <Label htmlFor="offer-message">{t("rd.message")}</Label>
                    <Textarea id="offer-message" placeholder={t("rd.messagePlaceholder")} value={offerMessage} onChange={(e) => setOfferMessage(e.target.value)} className="mt-1" rows={3} />
                  </div>
                  <div className="flex gap-2">
                    <Button variant="hero" className="gap-1.5" onClick={handleSubmitOffer} disabled={submittingOffer}>
                      {submittingOffer ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} {t("rd.submit")}
                    </Button>
                    <Button variant="ghost" onClick={() => setShowOfferForm(false)}>{t("rd.cancel")}</Button>
                  </div>
                </CardContent>
              </Card>
            )}
            ...
            {user && (isOwner || offers.length > 0) && (
              <div>
                <h2 className="font-display text-xl font-bold">{t("rd.offers")} ({offers.length})</h2>
                            ...
                            <div>
                              ...
                              {!isOwner && offer.worker_id === user?.id && (
                                <div className="mt-2 flex items-center gap-2">
                                  <span className="text-xs text-muted-foreground">{t("rd.rateCustomer")}</span>
                                  <StarRating
                                  ...
                                  {pendingRating[request.user_id] && pendingRating[request.user_id] !== ratingWorkerMap[request.user_id] && (
                                    ...
                                    >
                                      {submittingRating === request.user_id ? <Loader2 className="h-3 w-3 animate-spin" /> : t("rd.save")}
                                    </Button>
                                  )}
                                  {ratingWorkerMap[request.user_id] && !pendingRating[request.user_id] && (
                                    <span className="text-xs text-primary font-medium">{t("rd.rated")}</span>
                                  )}
                                </div>
                              )}
                              ...
                              {isOwner && (
                                <div className="mt-2 flex items-center gap-2">
                                  <span className="text-xs text-muted-foreground">{t("rd.rateWorker")}</span>
                                  <StarRating
                                  ...
                                  {pendingRating[offer.worker_id] && pendingRating[offer.worker_id] !== ratingWorkerMap[offer.worker_id] && (
                                    ...
                                    >
                                      {submittingRating === offer.worker_id ? <Loader2 className="h-3 w-3 animate-spin" /> : t("rd.save")}
                                    </Button>
                                  )}
                          ...
                          <div className="text-right shrink-0">
                            ...
                            <div className="mt-1 flex gap-1.5">
                              ...
                              {offer.status === "accepted" && (
                                <Badge variant="status" className="bg-primary/10 text-primary">{t("rd.accepted")}</Badge>
                              )}
                ...
                <Card className="mt-6 border-primary/20 shadow-card">
                  <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                      <CheckCircle2 className="h-5 w-5 text-primary" /> {t("rd.workCompletion")}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between rounded-lg border p-3">
                        <span className="text-sm font-medium">{t("rd.customer")}</span>
                        {customerDone ? (
                          <Badge className="bg-primary/10 text-primary gap-1"><CheckCircle2 className="h-3 w-3" /> {t("rd.confirmed")}</Badge>
                        ) : isOwner ? (
                          <Button size="sm" variant="hero" className="gap-1" onClick={() => handleConfirm("customer")}>
                            <CheckCircle2 className="h-3.5 w-3.5" /> {t("rd.markComplete")}
                          </Button>
                        ) : (
                          <Badge variant="outline" className="text-muted-foreground">{t("rd.pending")}</Badge>
                        )}
                      </div>
                      <div className="flex items-center justify-between rounded-lg border p-3">
                        <span className="text-sm font-medium">{t("rd.worker")}</span>
                        {workerDone ? (
                          <Badge className="bg-primary/10 text-primary gap-1"><CheckCircle2 className="h-3 w-3" /> {t("rd.confirmed")}</Badge>
                        ) : isWorkerOnThis ? (
          ...
          <div className="space-y-4">
            <Card className="shadow-card">
              <CardHeader><CardTitle className="text-lg">{t("rd.budget")}</CardTitle></CardHeader>
              <CardContent>
                <div className="flex items-center gap-1 font-display text-3xl font-bold">
                  {request.budget ? `${Number(request.budget).toLocaleString()} ₸` : t("rd.negotiable")}
                </div>
                <p className="mt-1 text-sm text-muted-foreground">{t("rd.initialPrice")}</p>
              </CardContent>
            </Card>

            <Card className="shadow-card">
              <CardHeader><CardTitle className="text-lg">{t("rd.postedBy")}</CardTitle></CardHeader>
              <CardContent>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold">{isOwner ? t("rd.you") : t("rd.customer")}</p>
                    {ownerAvgRating > 0 && (
                      <StarRating rating={ownerAvgRating} size="sm" showValue />
                    )}
                    <p className="text-xs text-muted-foreground">{t("rd.contactAfter")}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            {canMakeOffer && !alreadyOffered && !showOfferForm && (
              <Button variant="hero" className="w-full gap-2" onClick={() => setShowOfferForm(true)}>
                <HardHat className="h-4 w-4" /> {t("rd.makeOffer")}
              </Button>
            )}

            {canMakeOffer && alreadyOffered && (
              <Button variant="outline" className="w-full gap-2" disabled>
                <CheckCircle2 className="h-4 w-4" /> {t("rd.offerSubmitted")}
              </Button>
            )}
            ...
            {user && !isOwner && ["assigned", "in_progress"].includes(request.status) && (() => {
              ...
              return request.worker_confirmed_complete ? (
                <Button variant="outline" className="w-full gap-2" disabled>
                  <CheckCircle2 className="h-4 w-4" /> {t("rd.youConfirmed")}
                </Button>
              ) : (
                <Button variant="hero" className="w-full gap-2" onClick={async () => {
                  ...
                  <CheckCircle2 className="h-4 w-4" /> {t("rd.markWorkComplete")}
                </Button>
              );
            })()}
            ...
            {!user && (
              <Link to="/login">
                <Button variant="hero" className="w-full gap-2">
                  <MessageSquare className="h-4 w-4" /> {t("rd.loginOffer")}
                </Button>
              </Link>
            )}
          </div>
