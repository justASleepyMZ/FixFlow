...
const Navbar = () => {
  ...
  const roleConfig: Record<string, { label: string; icon: React.ElementType; color: string }> = {
    user: { label: t("role.user"), icon: User, color: "bg-primary text-primary-foreground" },
    worker: { label: t("role.worker"), icon: HardHat, color: "bg-secondary text-secondary-foreground" },
    company: { label: t("role.company"), icon: Building2, color: "bg-accent text-accent-foreground" },
    admin: { label: t("role.admin"), icon: ShieldCheck, color: "bg-destructive text-destructive-foreground" },
  };

  const links = [
    { to: "/", label: t("nav.home") },
    { to: "/requests", label: t("nav.browse") },
    ...(user && userRole !== "admin" ? [{ to: "/my-requests", label: t("nav.myRequests") }] : []),
    ...(userRole === "admin" ? [{ to: "/admin", label: t("nav.admin") }] : []),
          ...
          {user ? (
            <>
              ...
              <Button variant="ghost" size="sm" onClick={() => signOut()}>
                <LogOut className="mr-1.5 h-3.5 w-3.5" /> {t("nav.signout")}
              </Button>
            </>
          ) : (
            <>
              <Link to="/login"><Button variant="ghost" size="sm">{t("nav.login")}</Button></Link>
              <Link to="/register"><Button variant="hero" size="sm">{t("nav.signup")}</Button></Link>
            </>
          )}
        </div>
