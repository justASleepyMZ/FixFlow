...
export default function SupportChat() {
  ...
  const SUGGESTIONS = [t("sc.s1"), t("sc.s2"), t("sc.s3"), t("sc.s4")];
        ...
        {open && (
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b bg-gradient-hero p-4 text-primary-foreground">
              ...
              <div>
                <h3 className="font-display text-sm font-semibold">{t("sc.title")}</h3>
                <p className="text-xs opacity-80">{t("sc.subtitle")}</p>
              </div>
            </div>
            ...
            <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto p-4">
              ...
              {messages.length === 1 && !isLoading && (
                <div className="space-y-2 pt-2">
                  <p className="text-xs font-medium text-muted-foreground">{t("sc.suggested")}</p>
                  {SUGGESTIONS.map((s) => (
                    <button
                      key={s}
                      onClick={() => send(s)}
